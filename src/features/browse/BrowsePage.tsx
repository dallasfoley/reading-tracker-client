...
export default function BrowsePage() {
  ...
  return (
    ...
    >
      <form
        className="flex flex-col gap-3 rounded-lg border border-zinc-800 bg-zinc-900/70 p-3 sm:flex-row"
        onSubmit={handleSubmit}
      >
        <label className="relative flex-1">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-500" />
          <Input
            className="h-10 border-zinc-700 bg-zinc-950 pl-9 text-zinc-100"
      ...
      <section>
        {submittedQuery.length === 0 ? (
          ...
          <p className="rounded-md border border-zinc-800 bg-zinc-900/70 px-4 py-8 text-center text-sm text-zinc-400">
...
function BookSearchResult({
  ...
  return (
    <li className="flex min-h-48 gap-4 rounded-lg border border-zinc-800 bg-zinc-900/70 p-4">
      <BookCover src={book.coverUrl} title={book.title} />
      <div className="flex min-w-0 flex-1 flex-col">
        <h2 className="line-clamp-2 text-base font-semibold text-zinc-100">
          {book.title}
        </h2>
        <p className="mt-1 line-clamp-1 text-sm text-zinc-400">{book.author}</p>
        <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-zinc-500">
          <div>
            <dt>Published</dt>
            <dd className="text-zinc-300">{book.yearPublished ?? 'Unknown'}</dd>
          </div>
          <div>
            <dt>Pages</dt>
            <dd className="text-zinc-300">{book.pageCount ?? 'Unknown'}</dd>
          </div>
        </dl>
        <Button
...
function BookCover({ src, title }: { src: string | null; title: string }) {
  if (src) {
    return (
      <img
        alt={`${title} cover`}
        className="h-36 w-24 shrink-0 rounded-md object-cover ring-1 ring-zinc-700"
        src={src}
      />
    )
  }
  ...
  return (
    <div className="flex h-36 w-24 shrink-0 items-center justify-center rounded-md bg-zinc-800 ring-1 ring-zinc-700">
      <BookOpen className="size-8 text-zinc-500" />
    </div>
  )
}
...
function EmptySearchState() {
  return (
    <div className="flex min-h-72 flex-col items-center justify-center rounded-lg border border-dashed border-zinc-800 bg-zinc-900/40 p-8 text-center">
      <BookOpen className="mb-3 size-9 text-zinc-500" />
      <h2 className="text-lg font-medium">Search Open Library</h2>
  ...
  return (
    <ul className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, index) => (
        <li
          className="flex min-h-48 gap-4 rounded-lg border border-zinc-800 bg-zinc-900/70 p-4"
          key={index}
        >
          <div className="h-36 w-24 shrink-0 animate-pulse rounded-md bg-zinc-800" />
          <div className="flex flex-1 flex-col gap-3">
            <div className="h-5 w-4/5 animate-pulse rounded bg-zinc-800" />
            <div className="h-4 w-2/3 animate-pulse rounded bg-zinc-800" />
            <div className="mt-auto h-8 w-20 animate-pulse rounded bg-zinc-800" />
          </div>
        </li>
      ))}
    </ul>
